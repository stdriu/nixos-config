...
THEME_DIR="$HOME/.local/share/icons/Papirus-Dark"
...
if [ ! -d "$THEME_DIR" ]; then
  echo "No writable Papirus-Dark copy at $THEME_DIR"
  echo "Run: mkdir -p ~/.local/share/icons && cp -rL ~/.nix-profile/share/icons/Papirus-Dark ~/.local/share/icons/"
  exit 1
fi
...
papirus-folders -t "$THEME_DIR" -C "$color" -u
